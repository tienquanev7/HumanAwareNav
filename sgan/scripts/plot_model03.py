...
def ploot():
    ...
    # Assuming total_aa and total_bb are lists of numpy arrays

    # Define a list to hold all x and y values
    x_values = []
    y_values = []

    # Iterate over total_aa and total_bb
    for i in range(4):
        x_values.append(total_aa[i][:, 0])
        y_values.append(total_aa[i][:, 1])
        x_values.append(total_bb[i][:, 0])
        y_values.append(total_bb[i][:, 1])

    # Print all x and y values
    for i in range(len(x_values)):
        print(f'x{i+1} = {x_values[i]}')
        print(f'y{i+1} = {y_values[i]}')
